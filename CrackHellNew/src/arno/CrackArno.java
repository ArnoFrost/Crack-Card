...
* 类说明 4.29——更新优化分割方式，从逗号改为空格加快输入效率
*/ 
...
//		int words2 = 0x4369;
		...
		StringBuffer b = new StringBuffer();		//用于追加字符串的关键
...
//					judge[i]=ID[i]^key[i];
				...
				b.append(String.format("33 90 "));
				b.append(String.format(fixedValue));			//固定控制位 
				b.append(String.format(fixedValue2));
		...
		case 11:
			try {
				for (int i = 0; i < key4.length; i++) {
					b.append(String.format("%x"+" ",key4[i]));
				}
				b.append(String.format(fixedValue+" "));
				for (int i = 0; i < key4.length; i++) {
